...
router.post("/accountRegister", (req, res, next) => {
  if (req.body.role && req.body.reg_id && req.body.password && req.body.token) {
    ...
    let fquery = conn.format(query, [tableName.substring(3), colName, reg_id]);
                ...
                if (password.length >= 8 && password.length <= 15) {
                  ...
                  myCrypto
                    ...
                    .then((keys) => {
                      ...
                      let encPrivateKey = keys.encPrivateKey;
                      let relayPassword = myCrypto.generateRelayPassword();
                  ...
                  res.status(400).json({
                    error:
                      "Password length should be between 8 and 15, included",
                  });
                }
              } else {
                res.status(400);
